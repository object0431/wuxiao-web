<template>
  <div>
    ...
    <div
      ...
      :style="isWorkBench?'left:20px;width: calc(100vw - 60px);':''"
    >
      <i
        class="el-icon-close"
      ...
      >
        ...
        >
          <el-col :span="8">
            <el-select
              v-model="handleForm.opinionSelect"
              placeholder="请选择"
              ref="custom-drop"
              @change="(e) => {
            ...
            >
              ...
              >
                ...
                >
                  <span style="padding: 10px 0;margin: 0 auto;text-align: center;color: #999;font-size: 14px;cursor: default;">请添加意见内容
                  </span>
                </el-option>
                <div v-else>
                  <el-option
        ...
        <el-form-item
          v-for="(item, index) in handleForm.approvalInfoList"
          :label="'办理步骤' + (index + 1)"
          :key="item.nodeCode"
          required
        >
          ...
          <el-col :span="8">
            <el-form-item
              class="approval-display" :class="item.apprNumber != '1' && 'isvisible'"
              :prop="'approvalInfoList.' + index + '.approveName'"
              :rules="{
              required: true, message: '审核人不能为空', trigger: 'change'
            }"
            >
              <el-select
                v-model="handleForm.approvalInfoList[index].approveName"
                placeholder="请选择"
                :popper-append-to-body="false"
                :ref="'unitTreeSelect' + index"
              >
                <el-option
                  v-for="item1 in item.officerList"
                  :key="item1.staffId"
                  :label="item1.staffName"
                  :value="item1.staffName"
                ></el-option>
              </el-select>
            </el-form-item>
            <div class="approval-display" :class="item.apprNumber == '1' && 'isvisible'" v-for="item2 in item.officerList" :key="item2.staffId" >
...
export default {
  ...
  methods: {
    ...
    async getApprovalInfo() {
      try {
        const res = await getApprovalInfoAll({
    ...
    onSubmit() {
      if(!this.isSubMit){
        this.$message.error(this.tipValue || '未找到审批人员信息，请联系管理员配置')
        return
      }
      this.$refs.handleForm.validate(async (valid) => {
        if (valid) {
          this.$confirm(
            `您确定要提交给${
              this.handleForm.approvalInfoList[0].approveName
            }吗？`,
            "提示",
            {
            ...
            this.handleForm.approvalInfoList.map((item) => {
              //查找审批人id
              if(item.apprNumber == '1'){
                let obj = item.officerList.find(
                  (item1) => item1.staffName == item.approveName
                );

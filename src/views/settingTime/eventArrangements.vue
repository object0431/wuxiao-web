<template>
    <!-- 活动安排 -->
    <div class="approval-eventArrangements-page">
        <div class="head-title">查询</div>
        <div class="query">
...
<script>
import { activityInfoQueryActivity, activityInfoDeleteActivity } from "@/assets/api/resultManage";
export default {
    name: "eventArrangements",
    components: {},
    data() {
        return {
            type: "add",
    ...
    methods: {
        ...
        async queryApprovalApply() {
            ...
            const res = await activityInfoQueryActivity({
                reqParam: {
                    ...this.form,
                },
                ...this.pageParam,
        ...
        goView(id){
            this.$router.push({ name: "eventArrangementsNewsView", params: {type: "HD", id: id }, });
        },
        //单独删除申请
        async deletePersonalTrainingApply1(id) {
            let formData = new FormData()
            formData.append('id', id)
            const res = await activityInfoDeleteActivity(formData);
            if (res.rspCode == "0000") {
        ...
        //新增
        goAdd() {
            this.$router.push({ name: "eventArrangementsNewsAdd", params: { type: "HD" }, });
        },
...
.approval-eventArrangements-page {
